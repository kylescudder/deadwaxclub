...
- Record creation is enforced by `record_creation_quotas` and the `records_enforce_creation_limit` trigger (`0027_enforce_record_creation_limit.sql`), not by the client count. It derives `created_by` from `auth.uid()`, blocks reassignment, serializes each creator on their profile row, counts every genuinely new record for life (including soft- and hard-deleted records), and bypasses the cap only for `profiles.is_premium_account` or a verified active `iap_entitlements` mirror. The migration locks `public.records` before historical attribution and retains that lock through quota seeding and trigger installation; Supabase runs the migration transactionally, so a write cannot fall into a backfill/enforcement gap. Quota rows have no client RLS policies and must never be added to PowerSync streams.
- `SupabaseConnector` fetches one CRUD entry at a time. A `records` PUT rejected with SQLSTATE `DW001` is terminal: retire the local record/dependants through PowerSync-managed tables, complete that one batch entry, and present the paywall. Throw network, authentication, and unknown failures so PowerSync retries them; never complete those entries.
