...
- StoreKit's local `VerificationResult.verified` is device evidence only. `BillingRepository.isSubscribed` grants database-backed unlimited creation only after `iap-sync-transaction` has mirrored an Apple-cryptographically-verified active JWS; it retries failed mirrors and surfaces a temporary verification error instead of promising an unlock. Both IAP Edge Functions use Apple's `SignedDataVerifier` with online certificate checks and validate the bundle ID, environment, product ID, and app-account token before changing `iap_entitlements`.
