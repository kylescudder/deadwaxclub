...
/// Steps a newly signed-in user might see, in display order. Empty array
/// means everything's already done.
///
...
final class OnboardingCoordinator: ObservableObject {
    ...
    /// Every step the user still needs to see, in display order. The
    /// onboarding sheet is shown when this is non-empty and dismisses when
    /// it becomes empty.
    @Published private(set) var pendingSteps: [OnboardingStep] = []

    var isOnboarding: Bool { !pendingSteps.isEmpty }

    /// Decide what to show right now given current profile + token state.
    /// Called whenever the user signs in or completes a step.
    /// `profileLoaded` is the gate that prevents flashing the sheet before
    /// the local DB has had a chance to surface the row.
    func reconcile(profileLoaded: Bool, profileDisplayName: String?, hasDiscogsToken: Bool, notificationsAuthorized: Bool) {
        ...
        _ = profileDisplayName // unused; kept in the signature for callers
        var steps: [OnboardingStep] = []
        if !discogsTokenSeen && !hasDiscogsToken {
            steps.append(.discogsToken)
        }
        if !notificationsSeen && !notificationsAuthorized {
            steps.append(.enableNotifications)
        }
        pendingSteps = steps
    }
