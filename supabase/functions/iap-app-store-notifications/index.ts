...
serve(async (req) => {
  try {
    ...
    const status = entitlementStatus(expiresAt, revokedAt, verified.notificationType);
    const entitlement = {
    ...
    const { error } = await supabase.from("iap_entitlements")
      .upsert({ user_id: transaction.appAccountToken, ...entitlement }, { onConflict: "user_id" });
