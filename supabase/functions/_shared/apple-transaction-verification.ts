...
const BUNDLE_ID = Deno.env.get("APPLE_BUNDLE_ID") ?? "com.deadwaxclub.app";
const PRODUCT_ID = "club.deadwax.supporter.monthly";
...
function appleRootCertificates(): Buffer[] {
  ...
  try {
    ...
    throw new AppleVerificationError("APPLE_ROOT_CERTIFICATES_BASE64 must be a JSON array");
  }
  if (!Array.isArray(encodedRoots) || encodedRoots.length === 0 ||
      encodedRoots.some((root) => typeof root !== "string" || root.length === 0)) {
    throw new AppleVerificationError("No Apple root certificates are configured");
  }
...
function verifier(environment: Environment): SignedDataVerifier {
  ...
  if (environment === Environment.PRODUCTION && !Number.isSafeInteger(appAppleId)) {
    throw new AppleVerificationError("APPLE_APPLE_ID must be a valid App Store numeric ID");
  }
...
function verifiedTransaction(payload: Record<string, unknown>, environment: "sandbox" | "production"): VerifiedAppleTransaction {
  if (payload.bundleId !== BUNDLE_ID) {
    throw new AppleVerificationError("Transaction bundle ID does not match this app");
  }
  ...
  if (declaredEnvironment !== environment) {
    throw new AppleVerificationError("Transaction environment does not match its verifier");
  }
  ...
  if (typeof originalTransactionId !== "string" || originalTransactionId.length === 0) {
    throw new AppleVerificationError("Transaction is missing its original transaction ID");
  }
  return {
    appAccountToken: normalizeUUID(payload.appAccountToken, "Transaction appAccountToken"),
...
async function verifyWithEnvironment<T>(
...
): Promise<{ value: T; environment: "sandbox" | "production" }> {
  const candidates: Array<[Environment, "sandbox" | "production"]> = [
  ...
  throw new AppleVerificationError("Apple could not verify the signed transaction");
}

export async function verifyAppleTransaction(signedTransactionInfo: unknown): Promise<VerifiedAppleTransaction> {
  if (typeof signedTransactionInfo !== "string" || signedTransactionInfo.split(".").length !== 3) {
    throw new AppleVerificationError("signedTransactionInfo must be a compact JWS");
  }
  const result = await verifyWithEnvironment((candidate) => candidate.verifyAndDecodeTransaction(signedTransactionInfo));
  return verifiedTransaction(result.value as Record<string, unknown>, result.environment);
}
...
export async function verifyAppleNotification(signedPayload: unknown): Promise<{
  ...
  const result = await verifyWithEnvironment((candidate) => candidate.verifyAndDecodeNotification(signedPayload));
  const notification = result.value as AppleNotification;
  const signedTransactionInfo = notification.data?.signedTransactionInfo;
  if (!signedTransactionInfo) {
    throw new AppleVerificationError("Verified notification does not contain transaction information");
  }
  ...
  if (transaction.environment !== result.environment) {
    throw new AppleVerificationError("Notification and transaction environments differ");
  }
